(function (global, factory) {
  ...
  function visavailChart (custom_options, dataset) {
    const d3 = window.d3 ? window.d3 : typeof require !== 'undefined' ? require('d3') : undefined
    const moment = window.moment ? window.moment : typeof require !== 'undefined' ? require('moment') : undefined
    ...
    function convertMomentToStrftime (momentFormat) {
      const replacements = { ddd: 'a', dddd: 'A', MMM: 'b', MMMM: 'B', lll: 'c', DD: 'd', D: 'e', 'YYYY-MM-DD': 'F', HH: 'H', H: 'H', hh: 'I', h: 'I', DDDD: 'j', DDD: '-j', MM: 'm', M: '-m', mm: 'M', m: '-M', A: 'p', a: 'P', ss: 'S', s: '-S', E: 'u', d: 'w', WW: 'W', ll: 'x', LTS: 'X', YY: 'y', YYYY: 'Y', ZZ: 'z', z: 'Z', SSS: 'L', '%': '%' }
      const tokens = momentFormat.split(/( |\/|:|,|\]|\[|\.)/)

      const strftime = tokens.map(function (token) {
        // Replace strftime tokens with moment formats
        if (token[0] == ':' || token[0] == '/' || token[0] == ' ' || token[0] == ',') { return token } else
        if (token[0] == '[' || token[0] == ']') { return } else
        if (replacements.hasOwnProperty(token)) { return '%' + replacements[token] }
    ...
    function periodInLocal () {
      if (date_format_local.LTS.indexOf('a') > -1 || date_format_local.LTS.indexOf('A') > -1) { return true }
      return false
    }
    ...
    if (!custom_options.hasOwnProperty('width')) { options.width = document.getElementById(options.id_div_graph).offsetWidth }
    ...
    function chart (selection) {
      ...
      selection.each(function drawGraph (dataset) {
        ...
        const div = d3.select('#' + options.id_div_container).append('div')
        ...
        const yTitlediv = d3.select('#' + options.id_div_container).append('div')
        ...
        if (!options.oldwidth) { options.oldwidth = width }
        let maxPages = 0
        let startSet
        let endSet
        ...
        const height = options.graph.height * noOfDatasets + options.line_spacing * noOfDatasets - 1

        // check how data is arranged
        for (let i = 0; i < dataset.length; i++) {
          if (dataset[i].description) { options.tooltip.description = true }
          if (dataset[i].data[0] != null && dataset[i].data[0].length == 3) {
            options.defined_blocks = true
            if (!options.custom_categories && !Number.isInteger(dataset[i].data[0][1])) { options.custom_categories = true }
            break
          }
        }
        ...
        const format_utc_time = d3.utcParse('%Y-%m-%d %H:%M:%S')
        const format_utc = d3.utcParse('%Y-%m-%d')
          ...
          d.data.forEach(function (d1) {
            ...
            if (!(d1[2] instanceof Date)) {
              if (!options.defined_blocks) {
                ...
                } else { throw new Error('Defined block true but dataset not correct') }
              }
            }
          })
        })
        ...
        let startDate = moment().year(2999).toDate()
        let endDate = moment().year(0).toDate()
        ...
        dataset.forEach(function (series, seriesI) {
          ...
          series.data.forEach(function (d, i) {
            ...
            if (d[2] >= endDate) { endDate = d[2] }
            if (d[0] <= startDate) { startDate = d[0] }
        ...
        if (options.display_date_range && (options.display_date_range[0] || options.display_date_range[1])) {
          if (options.display_date_range[0]) {
            if (!(options.display_date_range[0] instanceof Date)) {
              ...
              if (options.date_in_utc) { startDate = moment.utc(options.display_date_range[0]).toDate() } else { startDate = options.display_date_range[0] }
            }
          }
          if (options.display_date_range[1]) {
            if (!(options.display_date_range[1] instanceof Date)) {
              ...
              if (options.date_in_utc) { endDate = moment.utc(options.display_date_range[1]).toDate() } else { endDate = options.display_date_range[1] }
            }
          }
        }
        ...
        // define axes
        const xAxis = d3.axisTop(options.xScale)
          .scale(options.xScale)
          .ticks(options.ticks_for_graph)
          .tickFormat(multiFormat)

        const subChartXAxis = d3.axisBottom(xScale2)
          .scale(xScale2)
          .ticks(options.ticks_for_graph)
          .tickFormat(multiFormat)
        ...
        if (options.show_y_title) {
          ...
          const y_axis_title = svg.select('#g_axis').append('g').attr('id', 'yAxis')
          ...
          if (options.y_title_tooltip.enabled) {
            ...
            function drawTitleTooltipWhenOver (obj, dataset, d, i) {
              const matrix = obj.getCTM().translate(obj.getAttribute('x'), obj.getAttribute('y'))
              yTitlediv.transition()
                .duration(options.y_title_tooltip.duration)
                .style('opacity', 1)

              yTitlediv.html(function () {
                const output = d.measure_description || ''
                return output
              })

              if (options.y_title_tooltip.type == 'right') {
                yTitlediv.style('left', function () {
                  if (options.y_title_tooltip.fixed) { return -1 * options.padding.left + 'px' }
        ...
        function redrawTooltipWhenMoved (pageX, pageY, obj) {
          div.style('left', function () {
            if (options.width < (pageX + div.property('offsetWidth') + options.tooltip.left_spacing)) { return ((pageX - div.property('offsetWidth')) - options.tooltip.left_spacing) + 'px' }
          ...
          if (options.tooltip.position === 'overlay') {
            if (document.getElementById(options.id_div_graph).offsetHeight < pageY + div.property('offsetHeight')) { div.style('top', (document.getElementById(options.id_div_graph).offsetHeight - div.property('offsetHeight')) + 'px') } else { div.style('top', (pageY) + 'px') }
          }
        }
        ...
        function redrawTooltipWhenOver (obj, dataset, pageX, pageY, d, i) {
          if (options.tooltip.hover_zoom.enabled) {
            d3.select(obj).transition()
              .duration(options.tooltip.duration)
              .attr('x', function (d) {
                if (options.graph.type == 'rhombus' || options.graph.type == 'circle') { return xForPoint(d, options.graph.width, options.xScale, options.line_spacing * options.tooltip.hover_zoom.ratio * 2) }
                return xForPoint(d, options.graph.width, options.xScale, 0)
              })
              .attr('width', function (d) {
                if (options.graph.type == 'rhombus' || options.graph.type == 'circle') { return widthForPoint(d, options.graph.width, options.xScale, options.line_spacing * options.tooltip.hover_zoom.ratio) }
                return widthForPoint(d, options.graph.width, options.xScale, 0)
              })
          ...
          div.html(function () {
            ...
            if (options.is_date_only_format && !options.tooltip.date_plus_time) {
              ...
              if (options.date_is_descending) { return output + moment(d[2]).format('l') }
              return output + moment(d[0]).format('l')
            } else {
              if (!options.tooltip.only_first_date) {
                ...
                if (options.date_is_descending) { return output + moment(d[2]).format('LTS') }
                return output + moment(d[0]).format('LTS')
              }
            }
          })
          ...
          if (options.tooltip.position === 'top') {
            div.style('top', function () {
              if (options.tooltip.hover_zoom.enabled) { return window.pageYOffset + matrix.f - options.tooltip.height - options.tooltip.hover_zoom.ratio * options.line_spacing + 'px' }
              return matrix.f - options.tooltip.height + 'px'
            })
              .style('height',
                function () {
                  if (options.tooltip.hover_zoom.enabled) { return options.tooltip.hover_zoom.ratio * options.line_spacing + options.graph.height + options.tooltip.height + 'px' }
        ...
        function emphasize (scale) {
          const xTicks = scale.ticks(options.ticks_for_graph)
        ...
        function zoomed () {
          ...
          if (e.sourceEvent && e.sourceEvent.type === 'brush') {
            if (!options.sub_chart.animation) { return }	// ignore zoom-by-brush
          }
          ...
          if (e.transform.k || e.transform.x) {
            ...
            // redraw tooltip
            if (e.sourceEvent && e.sourceEvent == 'touchmove') { redrawTooltipWhenMoved(e.sourceEvent.touches[0].layerX, e.sourceEvent.touches[0].layerY) } else if (e.sourceEvent) { redrawTooltipWhenMoved(e.sourceEvent.layerX, e.sourceEvent.layerY) }
            ...
            // } else
            if (options.sub_chart.enabled) { svg.select('#g_brush').call(brush.move, options.xScale.range().map(e.transform.invertX, e.transform)) }
        ...
        if (options.scale) {
          ...
          const newX = -((width * options.scale.k) * ((options.scale.x * -1) / (options.oldwidth * options.scale.k)))
          const new_translate = (newX - options.scale.x) / options.scale.k
          ...
          if (options.sub_chart.enabled) { svg.select('#g_brush').call(brush.move, xScale.range()) }
        }

        function xForPoint (d, graph_width, xScale, ratio) {
          let x_scale = xScale(d[0]) - ratio
          if (options.date_is_descending) { x_scale = xScale(d[2]) - ratio }
          if (isNaN(x_scale) || x_scale <= 0 || x_scale + ratio > width) { return 0 - ratio / 2 }
          if (options.graph.type == 'rhombus' || options.graph.type == 'circle') { return xScale(d[0]) - graph_width / 2 }

          return x_scale
        }

        function widthForPoint (d, graph_width, xScale, ratio) {
          let x_scale_d0 = xScale(d[0]) - ratio
          let x_scale_d2 = xScale(d[2]) + ratio
          ...
          if (isNaN(x_scale_d0) || isNaN(x_scale_d2) || (!options.date_is_descending && (x_scale_d2 - x_scale_d0) < 0) || x_scale_d2 <= 0 && x_scale_d0 <= 0) { return 0 }

          if (options.date_is_descending && (x_scale_d2 - x_scale_d0) < 0) { return (-1 * (x_scale_d2 - x_scale_d0)) }

          if (options.graph.type == 'rhombus' || options.graph.type == 'circle') {
            if (x_scale_d0 + ratio < 0) { return 0 - ratio }
            return graph_width + ratio
          }

          if (x_scale_d0 < 0 && x_scale_d2 > 0) { return x_scale_d2 > width ? width : x_scale_d2 - ratio }

          if (x_scale_d2 < 0 && x_scale_d0 > 0) { return x_scale_d0 > width ? width : x_scale_d0 - ratio }

          if (x_scale_d2 > width) { return width - x_scale_d0 < 0 ? 0 : (width - x_scale_d0 + ratio) } else { return x_scale_d2 - x_scale_d0 }
        }

        function transformForTypeOfGraph (d, xScale, graph_height, line_spacing, ratio) {
          const x_scale = xScale(d[0])
        ...
        function roundedRect () {
          if (options.graph.type == 'circle') { return 200000 }
          return 0
        }
      })
    ...
    chart.updateGraph = function (custom_option, dataset) {
      if (document.getElementById(options.id_div_graph) && document.getElementById(options.id_div_graph).innerHTML != '') {
        ...
        if (custom_option) { loadConfig(options, custom_options) }

        if (dataset) { return chart.createGraph(dataset) }
    ...
    chart.destroy = function (_) {
      if (document.getElementById(options.id_div_graph)) { document.getElementById(options.id_div_graph).innerHTML = '' }
      if (document.getElementById(options.id)) { document.getElementById(options.id).remove() }
